...
class AGLRFE(nn.Module):
	...
	def forward(self, x, with_activations=False):
		...
		if with_activations:
			return self.conv(torch.cat([res, x], dim = 1)), res
		else:
...
class USODFuseNet(nn.Module):
	...
	dilation_rates = [[1, 2, 3, 4, 5], [1, 2, 3, 4]]):
		...
		super(USODFuseNet, self).__init__()
		...
		if self.use_depth:
			...
			self.cvt_depth_features = DoubleConv((128 // factor), (128 * factorw) // factor) ## SODAWideNet++S Encoder
			self.lvl1_depth_features = DoubleConv(32, 32 * factorw)
			self.lvl2_depth_features = DoubleConv(64, 64 * factorw)

			self.hl3 = OutConv((128 * factorw) // factor, n_classes)
			self.hl2 = OutConv(64 * factorw, n_classes)
	...
	def forward(self, x, depth_features=None):
		...
		x1 = self.inc(x)
		if self.use_depth:
			depth_features_1 = self.lvl1_depth_features(depth_features[0])
			x1_hybrid = x1 + depth_features_1

			res1 = self.f1(x1_hybrid)
			ds1 = self.down1(x1_hybrid)
			x2 = self.comb1(res1, ds1)

			depth_features_2 = self.lvl2_depth_features(depth_features[1])
			x2_hybrid = x2 + depth_features_2

			res2 = self.f2(x2_hybrid)
			ds2 = self.down2(x2_hybrid)
			x3 = self.comb2(res2, ds2)

			depth_features_3 = self.cvt_depth_features(depth_features[2])
			x3_hybrid = x3 + depth_features_3
		...
		if (self.encoder_only) and (not self.deep_supervision):
			return x3
		...
		if self.deep_supervision:
			...
			if self.use_depth:
				...
				hybrid2_ = self.up3b(self.hl2(x2_hybrid))
				saliency.append(hybrid2_)

				hybrid3_ = self.up2b(self.hl3(x3_hybrid))
				saliency.append(hybrid3_)

			if self.encoder_only:
				return [x1, x2, x3], saliency

		if self.contour:

			decds2 = self.up3x(x3_hybrid)
			x = self.up3(self.combd2(x3_hybrid, decds2), x2_hybrid); logits_small = self.up3b(self.outs(x)); contour_small = self.up3b(self.outcontour2(x))
			...
			decds1 = self.up4x(x)
			x = self.up4(self.combd1(x, decds1), x1_hybrid)
			if self.deep_supervision:
